...
module('apps.dwarfcassowary.js.Hashtable').requires().toRun(function() {
	...
	Object.subclass('Hashtable', 'default category', {
		...
		escapingEach: function(callback) {
			var that = this;
				...
				// 2011-10-21 fbo context can be falsey
                                if (context && context.return) { return context.return; }
                                if (context && context.break) { break; }
                        }   
                },
		...
		clone: function() {
			var that = this;
			var clone = new Hashtable(this.hashingFunctionParam, this.equalityFunctionParam);
			clone.putAll(that);
			return clone;
		},
	});
